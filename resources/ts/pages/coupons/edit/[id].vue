...
const fetchCoupon = async () => {
  ...
  try {
    ...
    if (data) {
      ...
      discount_type.value = data.discount_type || 'percentage'
      discount_value.value = data.discount_value !== undefined ? data.discount_value : null
      start_date.value = data.start_date ? data.start_date.substring(0, 16).replace('T', ' ') : ''
      end_date.value = data.end_date ? data.end_date.substring(0, 16).replace('T', ' ') : ''
      is_unlimited.value = data.usage_limit === null || data.usage_limit === undefined
      usage_limit.value = data.usage_limit || null
      is_active.value = Boolean(data.is_active)
      notes.value = data.notes || ''
    }
  } catch (error) {
    console.error('Error fetching coupon details:', error)
    alert('تعذر تحميل بيانات الكوبون')
    router.push('/coupons')
  } finally {
...
const onSubmit = () => {
  refForm.value?.validate().then(async ({ valid }) => {
    if (valid) {
      loading.value = true
      try {
        await $api(`/coupons/${route.params.id}`, {
          method: 'PUT',
          body: {
        ...
        alert(error?.data?.message || 'حدث خطأ أثناء تعديل الكوبون')
      } finally {
        loading.value = false
      }
    }
...
<template>
  <div>
    <div v-if="fetching" class="p-12 text-center">
      ...
      <div class="mt-4 text-body-1">جاري تحميل بيانات الكوبون...</div>
    </div>
      ...
      <div class="d-flex flex-wrap justify-start justify-sm-space-between gap-y-4 gap-x-6 mb-6">
        <div class="d-flex flex-column justify-center">
          <h4 class="text-h4 font-weight-medium">
            تعديل بيانات الكوبون: <span class="text-primary">{{ code }}</span>
          </h4>
          <div class="text-body-1 text-medium-emphasis">
            تعديل الإعدادات والتواريخ والخصم أو الصلاحيات للرمز المخبري
          </div>
        </div>
