...
// Custom filter for VAutocomplete
...
// Computed filtered list for Quick Search Table
...
const fetchPackageData = async () => {
  ...
  try {
    const res = await $api(`/package-offers/${packageId}`)
    if (res.status && res.package_offer) {
      const data = res.package_offer
      ...
      originalPrice.value = data.original_price
      discountPrice.value = data.discount_price ?? ''
      image.value = data.image || ''
      if (data.image) imagePreview.value = data.image
      sortOrder.value = data.sort_order || 1
      isActive.value = data.is_active ?? true

      if (data.tests && Array.isArray(data.tests)) {
        selectedTestIds.value = data.tests.map((t: any) => t.id)
      }
    } else {
      errorMessage.value = 'تعذر العثور على بيانات الباقة المطلوبة.'
    }
  } catch (error) {
    console.error('Error fetching package data:', error)
    errorMessage.value = 'حدث خطأ أثناء جلب بيانات العرض من الخادم.'
  } finally {
...
// Update Form
const updatePackageOffer = async () => {
  loading.value = true
  errorMessage.value = ''
  ...
  try {
    ...
    if (res.status) {
      ...
      errorMessage.value = res.message || 'حدث خطأ أثناء تحديث العرض.'
    }
  } catch (error: any) {
    ...
    if (error && error.errors) {
      ...
      errorMessage.value = 'تعذر الاتصال بالخادم وحفظ التعديلات.'
    }
  } finally {
    loading.value = false
  }
...
<template>
  <div class="package-edit-page">
      ...
      <div class="d-flex flex-column justify-center">
        <div class="d-flex align-center gap-2 mb-1">
          ...
          <h4 class="text-h4 font-weight-bold">
            تعديل بيانات الباقة / العرض رقم (#{{ packageId }})
          </h4>
        </div>
        <div class="text-body-1 text-medium-emphasis">
          قم بتحديث اسم العرض وسعره وتغيير التحاليل المشمولة وحالة التفعيل في التطبيق
        </div>
      </div>
    ...
    <div v-if="initialLoading" class="text-center py-12">
      ...
      <div class="mt-4 text-body-1 text-medium-emphasis">جاري تحميل تفاصيل الباقة والتحاليل المشمولة...</div>
    </div>
              ...
              <VRow>
                ...
                <VCol cols="12">
                  <AppTextField
                    ...
                    placeholder="e.g. Comprehensive Thyroid Package"
                    :error-messages="validationErrors.name_en"
                  />
                </VCol>

                <VCol cols="12">
                  <label class="v-label mb-1 text-body-2 font-weight-medium">وصف الباقة ومميزاتها (بالعربية)</label>
                  <VTextarea
                ...
                <VCol cols="12">
                  <label class="v-label mb-1 text-body-2 font-weight-medium">وصف الباقة (بالإنجليزية - اختياري)</label>
                  <VTextarea
                    v-model="descriptionEn"
                    placeholder="English description..."
                    rows="3"
                    variant="outlined"
                    :error-messages="validationErrors.description_en"
                  />
                </VCol>
              </VRow>
            </VCardText>
        ...
        <VCol cols="12" md="4">
          ...
          <VCard title="صورة الباقة أو العرض">
            <VCardText>
              ...
              <!-- Live Preview -->
              <div v-if="imagePreview || image" class="text-center border rounded pa-3 bg-var-theme-background">
