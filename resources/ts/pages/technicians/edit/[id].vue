...
const technicianId = route.params.id as string
...
const fetchTechnician = async () => {
  ...
  try {
    const res = await $api(`/technicians/${technicianId}`)
    if (res.status && res.technician) {
      const t = res.technician
      name.value         = t.name || ''
      phone.value        = t.phone || ''
      address.value      = t.address || ''
      specialty.value    = t.specialty || ''
      hasTransport.value = t.has_transport
      hasEquipment.value = t.has_equipment
      notes.value        = t.notes || ''
      status.value       = t.status || 'active'
      idFrontImage.value    = t.id_front_image || null
      idBackImage.value     = t.id_back_image || null
      districtIdImage.value = t.district_id_image || null
    } else {
      errorMessage.value = 'لم يتم العثور على بيانات الفني'
    }
  } catch {
    errorMessage.value = 'تعذر جلب بيانات الفني'
  } finally {
...
const updateTechnician = async () => {
  ...
  try {
    ...
    if (res.status) {
      ...
      errorMessage.value = res.message || 'حدث خطأ أثناء تحديث بيانات الفني'
    }
  } catch (e: any) {
...
<template>
  <div>
    ...
    <div class="d-flex flex-wrap justify-start justify-sm-space-between gap-y-4 gap-x-6 mb-6">
      <div class="d-flex flex-column justify-center">
        <div class="d-flex align-center gap-2 mb-1">
          ...
          <h4 class="text-h4 font-weight-medium">تعديل بيانات الفني (#{{ technicianId }})</h4>
        </div>
        <div class="text-body-1 text-medium-emphasis">تحديث المعلومات الشخصية والوظيفية للفني الميداني</div>
      </div>
      <div class="d-flex gap-3 align-center">
        <VBtn variant="tonal" color="secondary" @click="router.push('/technicians')">إلغاء</VBtn>
        <VBtn color="primary" prepend-icon="tabler-device-floppy" :loading="loading || initialLoading" @click="updateTechnician">
          حفظ التعديلات
        </VBtn>
      </div>
    </div>
    ...
    <VCard v-if="initialLoading" class="py-12 text-center">
      ...
      <div class="text-h6 font-weight-medium">جاري تحميل بيانات الفني...</div>
    </VCard>
            ...
            <VCardText>
              <VRow>
                ...
                <VCol cols="12" md="6">
                  <AppTextField
                    v-model="password"
                    label="كلمة السر الجديدة (اتركها فارغة للإبقاء على السابقة)"
                ...
                <VCol cols="12">
                  ...
                  <VTextarea
                    v-model="notes"
                    placeholder="أي ملاحظات تخص الفني..."
                    rows="3"
                  />
                </VCol>
              </VRow>
          ...
          <VCard title="الحالة الوظيفية" class="mb-6">
            ...
            <VCardText>
              ...
              <div class="d-flex flex-column gap-2">
                <div
                  class="d-flex align-center gap-2 pa-3 rounded"
                  :style="{ background: status === 'active' ? 'rgba(var(--v-theme-success), 0.12)' : 'transparent' }"
                >
                ...
                <div
                  class="d-flex align-center gap-2 pa-3 rounded"
                  :style="{ background: status === 'on_leave' ? 'rgba(var(--v-theme-warning), 0.12)' : 'transparent' }"
                >
                ...
                <div
                  class="d-flex align-center gap-2 pa-3 rounded"
                  :style="{ background: status === 'suspended' ? 'rgba(var(--v-theme-error), 0.12)' : 'transparent' }"
                >
