...
class UpdateTubeTypeRequest extends FormRequest
{
    ...
    {
        $tube = $this->route('id') ?? $this->route('tubeType');
        $tubeId = is_object($tube) ? $tube->id : $tube;

        return [
            'name_ar'     => 'required|string|max:255|unique:tube_types,name_ar,' . $tubeId,
