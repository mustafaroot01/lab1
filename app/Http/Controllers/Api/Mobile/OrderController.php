...
class OrderController extends Controller
{
    ...
    public function myOrders(Request $request)
    {
        $orders = Order::where(function($q) use ($request) {
            ...
            ->with(['patient.district', 'patient.area', 'items', 'branch', 'technician'])
            ->withCount('items')
            ->latest()
            ->paginate(15);
    ...
    public function show(Request $request, $id)
    {
        $order = Order::where(function($q) use ($request) {
            ...
            ->where('id', $id)
            ->with(['patient.district', 'patient.area', 'items', 'branch', 'technician', 'coupon', 'results', 'statusLogs.changedBy'])
            ->firstOrFail();
